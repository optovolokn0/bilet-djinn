...
export default function ReaderCatalog() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Данные с бэкенда должны содержать available_count
  ...
  // Подсчет доступных книг (берем готовое значение с бэкенда)
  const counts = useMemo(() => {
    const map: Record<number, number> = {};
    allBookGroups.forEach(bg => {
      // Используем поле available_count, которое приходит с API
  ...
  return (
    <div className="book-catalog">
      <h2 className='book-catalog title'>Каталог книг</h2>
      ...
      {!isLoading && !loadError && items.length === 0 && (
        <div style={{ textAlign: 'center', padding: '20px' }}>
          {q ? 'Ничего не найдено.' : 'Каталог пуст.'}
        </div>
      )}
    </div>
  );
